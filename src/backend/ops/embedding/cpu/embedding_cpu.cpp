...
template <typename T> void embedding_(T* out, const int* index, const T* weight, size_t numel, size_t len) {
...
void embedding(std::byte* output, const std::byte* indices, const std::byte* weight, zedinferDataType_t type,
    ...
    switch (type) {
        case ZEDINFER_DTYPE_F32:
            return embedding_(reinterpret_cast<float*>(output), reinterpret_cast<const int*>(indices),
                              reinterpret_cast<const float*>(weight), numel, hidden_size);
        case ZEDINFER_DTYPE_F16:
            return embedding_(reinterpret_cast<zedinfer::fp16_t*>(output), reinterpret_cast<const int*>(indices),
                              reinterpret_cast<const zedinfer::fp16_t*>(weight), numel, hidden_size);
        case ZEDINFER_DTYPE_BF16:
            return embedding_(reinterpret_cast<zedinfer::bf16_t*>(output), reinterpret_cast<const int*>(indices),
                              reinterpret_cast<const zedinfer::bf16_t*>(weight), numel, hidden_size);
        default:
