...
// ----------------------------------------------------------------------
template <typename T>
__global__ void embedding_lookup_scalar_kernel(T* output, const T* table, const int* indices, size_t hidden_size) {
    ...
    for (size_t i = tid; i < hidden_size; i += blockDim.x) { output[out_offset + i] = table[table_offset + i]; }
}
...
// ----------------------------------------------------------------------
template <typename T>
__global__ void embedding_lookup_packed_kernel(T* output, const T* table, const int* indices, size_t hidden_size) {
...
void embedding(std::byte* output, const std::byte* indices, const std::byte* weight, zedinferDataType_t type,
    ...
    switch (type) {
        case ZEDINFER_DTYPE_F32:
            return embedding_lookup_packed_kernel<<<grid, block>>>(reinterpret_cast<float*>(output),
                                                                   reinterpret_cast<const float*>(weight),
                                                                   reinterpret_cast<const int*>(indices), hidden_size);
        case ZEDINFER_DTYPE_F16:
            return embedding_lookup_packed_kernel<<<grid, block>>>(reinterpret_cast<half*>(output),
                                                                   reinterpret_cast<const half*>(weight),
                                                                   reinterpret_cast<const int*>(indices), hidden_size);
        case ZEDINFER_DTYPE_BF16:
            return embedding_lookup_packed_kernel<<<grid, block>>>(reinterpret_cast<cuda_bfloat16*>(output),
