...
__device__ inline float2 prepare_scalars(float b_raw, float a_raw, float A_log, float dt_bias) {
    // beta = sigmoid(b)
    float beta = 1.0f / (1.0f + __expf(-b_raw));
    // softplus(x) = log(1 + exp(x));  use stable form via log1pf(exp(...))
    float sp = (a_raw + dt_bias) > 20.0f ? (a_raw + dt_bias) : __logf(1.0f + __expf(a_raw + dt_bias));
...
__device__ inline float dot_row(const float* S_row, const __nv_bfloat16* k_vec, int Dk, int lane_id,
                                int lanes_per_row) {
    float acc = 0.0f;
    for (int j = lane_id; j < Dk; j += lanes_per_row) { acc += S_row[j] * __bfloat162float(k_vec[j]); }
    // Warp reduction across lanes (warp-cooperative).
    for (int off = lanes_per_row / 2; off > 0; off >>= 1) { acc += __shfl_xor_sync(0xffffffff, acc, off); }
    return acc;
}
...
__device__ inline void update_row(float* S_row, const __nv_bfloat16* k_vec, float decay, float beta, float delta_d,
...
__device__ inline float readout_row(const float* S_row, const __nv_bfloat16* q_vec, int Dk, int lane_id,
                                    int lanes_per_row) {
    float acc = 0.0f;
    for (int j = lane_id; j < Dk; j += lanes_per_row) { acc += S_row[j] * __bfloat162float(q_vec[j]); }
    for (int off = lanes_per_row / 2; off > 0; off >>= 1) { acc += __shfl_xor_sync(0xffffffff, acc, off); }
    return acc;
}
