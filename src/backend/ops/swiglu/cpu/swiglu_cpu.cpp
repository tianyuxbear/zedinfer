...
void swiglu(std::byte *output, const std::byte *gate, const std::byte *up, zedinferDataType_t type, size_t numel) {
    ...
    case ZEDINFER_DTYPE_F32:
        return swiglu_(reinterpret_cast<float *>(output),
                       reinterpret_cast<const float *>(gate),
                       reinterpret_cast<const float *>(up),
                       numel);
    case ZEDINFER_DTYPE_F16:
        return swiglu_(reinterpret_cast<zedinfer::fp16_t *>(output),
                       reinterpret_cast<const zedinfer::fp16_t *>(gate),
                       reinterpret_cast<const zedinfer::fp16_t *>(up),
                       numel);
    case ZEDINFER_DTYPE_BF16:
        return swiglu_(reinterpret_cast<zedinfer::bf16_t *>(output),
                       reinterpret_cast<const zedinfer::bf16_t *>(gate),
                       reinterpret_cast<const zedinfer::bf16_t *>(up),
                       numel);
    default:
