...
#pragma omp parallel for
    for (size_t i = 0; i < nrow; ++i) {
        ...
        if constexpr (std::is_same_v<T, zedinfer::bf16_t>) {
            ...
            zedinfer::utils::bf16_to_fp32_batch(in_f32.data(), in_t, ncol);
            zedinfer::utils::bf16_to_fp32_batch(weight_f32.data(), weight, ncol);

            square_sum = sdot(in_f32.data(), in_f32.data(), ncol);
            ...
            zedinfer::utils::fp32_to_bf16_batch(out_t, out_f32.data(), ncol);
            ...
            zedinfer::utils::fp16_to_fp32_batch_f16c(in_f32.data(), in_t, ncol);
            zedinfer::utils::fp16_to_fp32_batch_f16c(weight_f32.data(), weight, ncol);
            ...
            zedinfer::utils::fp32_to_fp16_batch_f16c(out_t, out_f32.data(), ncol);

        } else {
            T rms{}, square_sum{}, avg_square_sum{};
            square_sum = sdot(in_t, in_t, ncol);
...
void rms_norm(std::byte *out, const std::byte *in, const std::byte *weight, float eps, zedinferDataType_t type, size_t nrow, size_t ncol) {
    ...
    case ZEDINFER_DTYPE_BF16:
        return rms_norm_(reinterpret_cast<zedinfer::bf16_t *>(out), reinterpret_cast<const zedinfer::bf16_t *>(in),
                         reinterpret_cast<const zedinfer::bf16_t *>(weight), eps, nrow, ncol);
    case ZEDINFER_DTYPE_F16:
        return rms_norm_(reinterpret_cast<zedinfer::fp16_t *>(out), reinterpret_cast<const zedinfer::fp16_t *>(in),
                         reinterpret_cast<const zedinfer::fp16_t *>(weight), eps, nrow, ncol);
    default:
