...
// Main entry point
// ============================================================================
void launch_linear_quantized_autotuned(cuda_bfloat16* output, const cuda_bfloat16* input, const void* weight_packed,
                                       const cuda_bfloat16* bias, const cuda_bfloat16* scales, const size_t M,
                                       const size_t N, const size_t K, const int num_bits, int group_size) {
    ...
    if (M == 1) {
        ...
        cuda_bfloat16* act_scales = reinterpret_cast<cuda_bfloat16*>(act_scale_storage->memory());

        launch_quantize_q8_row<cuda_bfloat16>(input, act_q, act_scales, 1, K, stream);
        ...
        if (num_bits == 4) {
            matvec_q4_0_q8_row_soa_kernel<cuda_bfloat16>
                <<<grid, block, 0, stream>>>(output, reinterpret_cast<const int32_t*>(weight_packed), scales, act_q,
                                             act_scales, bias, N, K, group_size);
        } else if (num_bits == 8) {
            matvec_q8_0_q8_row_soa_kernel<cuda_bfloat16>
    ...
    if (num_bits == 4) {
        ...
        num_act_groups = (act_group_size > 0 && act_group_size < (int)K) ? ((int)K / act_group_size) : 1;
    } else {
        ...
        act_group_size = (group_size > 0 && group_size < (int)K) ? group_size : (int)K;
        num_act_groups = (act_group_size > 0 && act_group_size < (int)K) ? ((int)K / act_group_size) : 1;
    }
    ...
    launch_quantize_q8_row_grouped<cuda_bfloat16>(input, act_q, act_scales, M, K, act_group_size, stream);
    ...
    if (num_bits == 4) {
        ...
        if (can_use_bk64 && blocks_large >= num_sms) {
            ...
            w4a8_gemm_soa_128x256_kernel<cuda_bfloat16><<<dim3(grid_large_x, grid_large_y), 256, smem_large, stream>>>(
            ...
            w4a8_gemm_soa_64x128_kernel<cuda_bfloat16><<<dim3(grid_small_x, grid_small_y), 128, smem_small, stream>>>(
                output, act_q, act_scales, reinterpret_cast<const uint8_t*>(weight_packed), scales, bias, M, N, K,
                group_size);
            ...
            if (split_k <= 1) {
                if (can_use_bk64) {
                    ...
                    w4a8_gemm_soa_64x128_kernel<cuda_bfloat16>
                        <<<dim3(grid_small_x, grid_small_y), 128, smem_small, stream>>>(
                            output, act_q, act_scales, reinterpret_cast<const uint8_t*>(weight_packed), scales, bias, M,
                            N, K, group_size);
                } else {
                    cudaFuncSetAttribute(w4a8_gemm_soa_64x128_kernel<cuda_bfloat16>,
                ...
                float* workspace = reinterpret_cast<float*>(workspace_storage->memory());
                ...
                w4a8_gemm_soa_64x128_splitk_kernel<cuda_bfloat16>
                    <<<dim3(grid_small_x, grid_small_y, split_k), 128, smem_splitk, stream>>>(
                        workspace, act_q, act_scales, reinterpret_cast<const uint8_t*>(weight_packed), scales, M, N, K,
                        group_size, split_k);

                launch_splitk_reduce<cuda_bfloat16>(output, workspace, bias, M, N, split_k, stream);
            }
        }
        ...
        if (can_use_bk64 && blocks_large >= num_sms) {
            ...
            if (split_k <= 1) {
                if (can_use_bk64) {
                    ...
                    w8a8_gemm_soa_64x128_kernel<cuda_bfloat16>
                        <<<dim3(grid_small_x, grid_small_y), 128, smem_small_opt, stream>>>(
                            output, act_q, act_scales, reinterpret_cast<const int8_t*>(weight_packed), scales, bias, M,
                            N, K, group_size);
                } else {
                    cudaFuncSetAttribute(w8a8_gemm_soa_64x128_kernel<cuda_bfloat16>,
                ...
                w8a8_gemm_soa_64x128_splitk_kernel<cuda_bfloat16>
                    <<<dim3(grid_small_x, grid_small_y, split_k), 128, smem_small_orig, stream>>>(
                        workspace, act_q, act_scales, reinterpret_cast<const int8_t*>(weight_packed), scales, M, N, K,
                        group_size, split_k);
