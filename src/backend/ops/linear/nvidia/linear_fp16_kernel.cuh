...
#pragma unroll
                for (int j = 0; j < 8; j++) {
                    if (gmem_k + j < K) {
                        ...
                        s_a[OFFSET(smem_m, load_a_smem_k + j, BK + APAD)] = __float2half(0.0f);
                    }
                }
            }
        }
...
#pragma unroll
                for (int j = 0; j < 8; j++) {
                    if (gmem_k + j < K) {
                        ...
                        s_b[OFFSET(smem_n, load_b_smem_k + j, BK + BPAD)] = __float2half(0.0f);
                    }
                }
            }
        }
...
#pragma unroll
    for (int j = 0; j < 4; j++) {
        ...
        bias_vals[j] = (global_n < N && bias) ? __half2float(bias[global_n]) : 0.0f;
    }
...
#pragma unroll
            ...
            if (global_m < M && global_n + 7 < N) {
                ...
                store_128b(&C[OFFSET(global_m, global_n, N)], load_128b(&s_c_half[wid * 256 + row * 16 + col]));
            } else if (global_m < M) {
                // Boundary case: element-wise store
#pragma unroll
                for (int c = 0; c < 8; c++) {
                    if (global_n + c < N) {
                        C[OFFSET(global_m, global_n + c, N)] = s_c_half[wid * 256 + row * 16 + col + c];
                    }
                }
            }
            __syncwarp();
