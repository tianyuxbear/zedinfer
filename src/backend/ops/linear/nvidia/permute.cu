...
template <typename T>
__global__ void permute_cols_kernel(const T *__restrict__ input,
...
tensor_t permute_cols(tensor_t input, tensor_t indices) {
    ...
    auto output = Tensor::create(input->shape(), input->dtype(),
                                 ZEDINFER_DEVICE_NVIDIA, input->deviceId());
    ...
    case ZEDINFER_DTYPE_F32:
        permute_cols_kernel<float><<<grid_size, kBlockSize, 0, stream>>>(
            reinterpret_cast<const float *>(input->data()), idx_ptr,
            reinterpret_cast<float *>(output->data()), total_elements, K);
        break;
    case ZEDINFER_DTYPE_F16:
        permute_cols_kernel<half><<<grid_size, kBlockSize, 0, stream>>>(
            reinterpret_cast<const half *>(input->data()), idx_ptr,
