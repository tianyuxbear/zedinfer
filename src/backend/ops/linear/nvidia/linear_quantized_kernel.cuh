...
__device__ __forceinline__ void unpack_row_int4x64(int8_t *__restrict__ dst,
...
__device__ __forceinline__ void unpack_row_int4x32(int8_t *__restrict__ dst,
                                                   const uint8_t *__restrict__ src) {
    const uint32_t *src4 = reinterpret_cast<const uint32_t *>(src);
    #pragma unroll
    for (int i = 0; i < 4; ++i) {
...
__device__ __forceinline__ int dp4a_q4_chunk(uint32_t packed_weight,
                                             int act_low, int act_high, int acc) {
...
__device__ __forceinline__ float block_reduce_sum(float value, float *shared_sum,
...
template <typename T>
__device__ __forceinline__ float load_or_zero(const T *values, int index, int limit) {
    return (index < limit && values) ? to_float(values[index]) : 0.0f;
}

template <typename T>
__device__ __forceinline__ void store_matvec_output(T *output, const T *bias,
                                                    size_t row_idx, float block_sum,
...
__device__ __forceinline__ void cp_async_16b(void *shared_ptr, const void *global_ptr,
                                             bool valid) {
    const uint32_t shared_addr = __cvta_generic_to_shared(shared_ptr);
    const int copy_bytes = valid ? 16 : 0;
    asm volatile("cp.async.ca.shared.global [%0], [%1], 16, %2;\n"
                 :: "r"(shared_addr), "l"(global_ptr), "r"(copy_bytes));
}
...
template <typename T>
__global__ void matvec_q4_0_q8_row_soa_kernel(
    T* __restrict__ out,
    const int32_t* __restrict__ weight_q,
    const T* __restrict__ weight_scales,
    const int8_t* __restrict__ act_q,
    const T* __restrict__ act_scales,
    const T* __restrict__ bias,
    size_t N, size_t K, int group_size)
{
...
template <typename T>
__global__ void matvec_q8_0_q8_row_soa_kernel(
    T* __restrict__ out,
...
template<typename T>
__global__ void w8a8_gemm_soa_64x128_kernel(
    T *__restrict__ C,
    ...
    const size_t M, const size_t N, const size_t K, const int group_size)
{
    ...
    auto load_tile = [&](int k_start, int buf) {
        ...
        cp_async_16b(
            &s_a[buf * a_buf_size + load_a_smem_m * A_STRIDE + load_a_smem_k],
            va0 ? &A_q[gmem_m_a * K + k_start + load_a_smem_k] : A_q, va0);

        bool va1 = (gmem_m_a < (int)M && k_start + load_a_smem_k + 16 < (int)K);
        cp_async_16b(
            &s_a[buf * a_buf_size + load_a_smem_m * A_STRIDE + load_a_smem_k + 16],
        ...
        for (int chunk = 0; chunk < 4; chunk++) {
            ...
            cp_async_16b(
                &s_b[buf * b_buf_size + tid * B_STRIDE + chunk * 16],
            ...
            for (int step = 0; step < 4; ++step) {
                ...
                for (int i = 0; i < 2; ++i)
                    wmma::load_matrix_sync(frag_a[step][i],
                        &s_a[a_buf * a_buf_size + (base_m + i * 16) * A_STRIDE + k_off],
                        A_STRIDE);
                #pragma unroll
                for (int j = 0; j < 4; ++j)
                    wmma::load_matrix_sync(frag_b[step][j],
                        &s_b[b_buf * b_buf_size + (base_n + j * 16) * B_STRIDE + k_off],
                        B_STRIDE);
                #pragma unroll
                for (int i = 0; i < 2; ++i)
                    #pragma unroll
                    for (int j = 0; j < 4; ++j)
                        wmma::mma_sync(frag_c_int32[i][j],
                                       frag_a[step][i], frag_b[step][j],
                                       frag_c_int32[i][j]);
            }
        }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            wmma::store_matrix_sync(&s_c_float[wid * 256], frag_c_fp32[i][j],
            ...
            for (int idx = lane_id; idx < 256; idx += 32) {
                int local_n = idx & 15;
                float b_val = (tile_n + local_n < (int)N && bias)
                              ? to_float(bias[tile_n + local_n]) : 0.0f;
                s_c_final[wid * 256 + idx] = from_float<T>(
                    s_c_float[wid * 256 + idx] + b_val);
            }
            __syncwarp();
...
// ============================================================================
template<typename T>
__global__ void w8a8_gemm_soa_128x256_kernel(
    T *__restrict__ C,
    const int8_t *__restrict__ A_q, const T *__restrict__ A_scales,
    const int8_t *__restrict__ B_q, const T *__restrict__ B_scales,
    const T *__restrict__ bias,
    ...
    for (int bk = 1; bk <= num_k_tiles; bk++) {
...

        {
            ...
            for (int step = 0; step < 4; ++step) {
                ...
                for (int i = 0; i < 4; ++i)
                    wmma::load_matrix_sync(frag_a[step][i],
                        &s_a[buf * a_buf_size + (base_m + i * 16) * A_STRIDE + k_off],
                        A_STRIDE);
                #pragma unroll
                for (int j = 0; j < 4; ++j)
                    wmma::load_matrix_sync(frag_b[step][j],
                        &s_b[buf * b_buf_size + (base_n + j * 16) * B_STRIDE + k_off],
                        B_STRIDE);
                #pragma unroll
                for (int i = 0; i < 4; ++i)
                    #pragma unroll
...
// ============================================================================
template<typename T>
__global__ void w4a8_gemm_soa_64x128_kernel(
    T *__restrict__ C,
    ...
    const int group_size)
{
    ...
    auto load_tile = [&](int k_start, int buf) {
        ...
        cp_async_16b(
            &s_b_packed[buf * bp_buf_size + tid * BK_PACKED],
            vb ? &B_q_packed[gmem_n_b * (K / 2) + k_start / 2] : B_q_packed, vb);
        cp_async_16b(
            &s_b_packed[buf * bp_buf_size + tid * BK_PACKED + 16],
            vb ? &B_q_packed[gmem_n_b * (K / 2) + k_start / 2 + 16] : B_q_packed, vb);
    };

    auto unpack_tile = [&](int src_buf, int dst_buf) {
        detail::unpack_row_int4x64(
            &s_b[dst_buf * b_buf_size + tid * B_STRIDE],
...
// ============================================================================
template<typename T>
__global__ void w4a8_gemm_soa_128x256_kernel(
    T *__restrict__ C,
    const int8_t *__restrict__ A_q,
    const T *__restrict__ A_scales,
    const uint8_t *__restrict__ B_q_packed,
    ...
    for (int bk = 1; bk <= num_k_tiles; bk++) {
        ...
        // Compute
        {
            ...
            for (int step = 0; step < 4; ++step) {
                ...
                for (int j = 0; j < 4; ++j)
                    wmma::load_matrix_sync(frag_b[step][j],
                        &s_b[(base_n + j * 16) * B_STRIDE + k_off],
                        B_STRIDE);
                #pragma unroll
                for (int i = 0; i < 4; ++i)
                    #pragma unroll
        ...
        if (bk < num_k_tiles) {
            ...
            int buf = bk & 1;
            detail::unpack_row_int4x64(&s_b[tid * B_STRIDE],
                                       &s_b_packed[buf * bp_buf_size + tid * BK_PACKED]);
            __syncthreads();
        }
    }
...
// ============================================================================
template <typename T>
__global__ void splitk_reduce_bias_kernel(
    ...
    const int split_k)
{
    ...
    for (int s = 0; s < split_k; ++s) {
        acc += workspace[static_cast<size_t>(s) * M * N + m * N + n];
    }
...
template <typename T>
void launch_splitk_reduce(
    T *output, const float *workspace, const T *bias,
    size_t M, size_t N, int split_k, cudaStream_t stream = nullptr)
{
    size_t total = M * N;
    int block = 256;
    int grid = (int)div_ceil(total, (size_t)block);
    splitk_reduce_bias_kernel<T><<<grid, block, 0, stream>>>(
        output, workspace, bias, M, N, split_k);
}

template<typename T>
__global__ void w8a8_gemm_soa_64x128_splitk_kernel(
    float *__restrict__ workspace,    // [split_k, M, N]
    const int8_t *__restrict__ A_q, const T *__restrict__ A_scales,
    const int8_t *__restrict__ B_q, const T *__restrict__ B_scales,
    const size_t M, const size_t N, const size_t K,
    const int group_size, const int split_k)
{
    ...
    auto issue_cp_async = [&](int8_t* smem_ptr, const int8_t* gmem_ptr, bool valid) {
        ...
        asm volatile("cp.async.ca.shared.global [%0], [%1], 16, %2;\n"
                     :: "r"(smem_addr), "l"(gmem_ptr), "r"(copy_bytes));
    };
    ...
    for (int bk = 1; bk <= num_k_tiles; bk++) {
        ...
        if (bk < num_k_tiles) {
            ...
            issue_cp_async(
                &s_a[OFFSET(load_a_smem_m, load_a_smem_k, BK + APAD) + next_idx * s_a_db_offset],
                v_a ? &A_q[gmem_m_a * K + k_global + load_a_smem_k] : A_q, v_a);

            v_b0 = (gmem_n_b0 < (int)N && k_global + load_b_smem_k < (int)K);
            issue_cp_async(
                &s_b[OFFSET(load_b_smem_n, load_b_smem_k, BK + BPAD) + next_idx * s_b_db_offset],
                v_b0 ? &B_q[gmem_n_b0 * K + k_global + load_b_smem_k] : B_q, v_b0);

            v_b1 = (gmem_n_b1 < (int)N && k_global + load_b_smem_k < (int)K);
            issue_cp_async(
                &s_b[OFFSET(load_b_smem_n + 64, load_b_smem_k, BK + BPAD) + next_idx * s_b_db_offset],
        ...
        // Compute current tile
        {
            ...
            for (int step = 0; step < 2; ++step) {
                ...
                for (int i = 0; i < 2; ++i)
                    wmma::load_matrix_sync(frag_a[step][i],
                        &s_a[OFFSET(base_m + i * 16, k_offset, BK + APAD) + curr_idx * s_a_db_offset], BK + APAD);
                #pragma unroll
                for (int j = 0; j < 4; ++j)
                    wmma::load_matrix_sync(frag_b[step][j],
                        &s_b[OFFSET(base_n + j * 16, k_offset, BK + BPAD) + curr_idx * s_b_db_offset], BK + BPAD);
                #pragma unroll
                for (int i = 0; i < 2; ++i)
                    #pragma unroll
                    for (int j = 0; j < 4; ++j)
...
// ============================================================================
template<typename T>
__global__ void w4a8_gemm_soa_64x128_splitk_kernel(
    ...
    const int group_size, const int split_k)
{
    ...
    // Prologue: load + unpack tile 0
    {
        ...
        cp_async_16b(&s_b_packed[tid * BK_PACKED],
                     vb ? &B_q_packed[gmem_n_b * (K / 2) + k_base / 2] : B_q_packed, vb);
    }
    asm("cp.async.commit_group;\n" ::);
    asm("cp.async.wait_group 0;\n" ::);
    __syncthreads();
    ...
    for (int bk = 1; bk <= num_k_tiles; bk++) {
        ...
        if (bk < num_k_tiles) {
            ...
            int buf = bk & 1;
            detail::unpack_row_int4x32(&s_b[tid * B_STRIDE],
                                       &s_b_packed[buf * bp_buf_size + tid * BK_PACKED]);
            __syncthreads();
        }
    }
