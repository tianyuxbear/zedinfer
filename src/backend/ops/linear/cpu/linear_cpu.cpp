...
void linear(std::byte *out, const std::byte *in, const std::byte *weight, const std::byte *bias, NeollmDataType_t type, size_t nrow, size_t ncol_out, size_t ncol_in) {
    ...
    case NEOLLM_DTYPE_BF16:
        return linear_(reinterpret_cast<neollm::bf16_t *>(out), reinterpret_cast<const neollm::bf16_t *>(in),
                       reinterpret_cast<const neollm::bf16_t *>(weight), reinterpret_cast<const neollm::bf16_t *>(bias), nrow, ncol_out, ncol_in);
    case NEOLLM_DTYPE_F16:
        return linear_(reinterpret_cast<neollm::fp16_t *>(out), reinterpret_cast<const neollm::fp16_t *>(in),
                       reinterpret_cast<const neollm::fp16_t *>(weight), reinterpret_cast<const neollm::fp16_t *>(bias), nrow, ncol_out, ncol_in);
    default:
