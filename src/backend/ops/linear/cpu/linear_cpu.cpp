...
template <typename T>
static void linear_quantized_impl(T* output, const T* input, const void* weight, const T* bias, const T* scales,
                                  int num_bits, int group_size, size_t M, size_t N, size_t K) {
    ...
    if constexpr (std::is_same_v<T, float>) {
        if (M == 1) {
            if (num_bits == 8) {
                zedinfer::ops::cpu::detail::vecmul_q8_soa(output, input, reinterpret_cast<const int8_t*>(weight), bias,
                                                          scales, static_cast<int>(N), static_cast<int>(K), group_size);
            } else {
                zedinfer::ops::cpu::detail::vecmul_q4_soa(output, input, reinterpret_cast<const int32_t*>(weight), bias,
                                                          scales, static_cast<int>(N), static_cast<int>(K), group_size);
            }
            return;
        }
        ...
        zedinfer::ops::cpu::detail::matmul_packed_unified(input, weight, output, static_cast<int>(M),
                                                          static_cast<int>(N), static_cast<int>(K), scales, num_bits,
                                                          group_size);
...
#pragma omp parallel for collapse(2)
        for (size_t m = 0; m < M; ++m) {
            for (size_t n = 0; n < N; ++n) { output[m * N + n] += bias[n]; }
        }
    } else if constexpr (std::is_same_v<T, zedinfer::bf16_t>) {
        std::vector<float> input_fp32(M * K);
        ...
        zedinfer::utils::bf16_to_fp32_batch(input_fp32.data(), input, M * K);
        zedinfer::utils::bf16_to_fp32_batch(scales_fp32.data(), scales, N * static_cast<size_t>(num_groups));
        ...
        linear_quantized_impl(output_fp32.data(), input_fp32.data(), weight, bias_ptr, scales_fp32.data(), num_bits,
                              group_size, M, N, K);
        zedinfer::utils::fp32_to_bf16_batch(output, output_fp32.data(), M * N);
    } else if constexpr (std::is_same_v<T, zedinfer::fp16_t>) {
        std::vector<float> input_fp32(M * K);
        ...
        zedinfer::utils::fp16_to_fp32_batch_f16c(input_fp32.data(), input, M * K);
        zedinfer::utils::fp16_to_fp32_batch_f16c(scales_fp32.data(), scales, N * static_cast<size_t>(num_groups));
...
void linear_quantized(std::byte* output, const std::byte* input, const std::byte* weight_packed, const std::byte* bias,
    ...
    if (num_bits != 4 && num_bits != 8) {
        throw std::runtime_error("linear_quantized CPU: only 4-bit and 8-bit weights are supported");
    }

    switch (type) {
        case ZEDINFER_DTYPE_F32:
            return linear_quantized_impl(reinterpret_cast<float*>(output), reinterpret_cast<const float*>(input),
                                         weight_packed, reinterpret_cast<const float*>(bias),
                                         reinterpret_cast<const float*>(scale), num_bits, group_size, M, N, K);
        case ZEDINFER_DTYPE_BF16:
            return linear_quantized_impl(
                reinterpret_cast<zedinfer::bf16_t*>(output), reinterpret_cast<const zedinfer::bf16_t*>(input),
                weight_packed, reinterpret_cast<const zedinfer::bf16_t*>(bias),
                reinterpret_cast<const zedinfer::bf16_t*>(scale), num_bits, group_size, M, N, K);
        case ZEDINFER_DTYPE_F16:
            return linear_quantized_impl(
                reinterpret_cast<zedinfer::fp16_t*>(output), reinterpret_cast<const zedinfer::fp16_t*>(input),
                weight_packed, reinterpret_cast<const zedinfer::fp16_t*>(bias),
                reinterpret_cast<const zedinfer::fp16_t*>(scale), num_bits, group_size, M, N, K);
        default:
            EXCEPTION_UNSUPPORTED_DATATYPE(type);
    }
}
