...
template <typename T>
static void permute_cols_impl(T *output, const T *input, const int32_t *indices,
                              size_t M, size_t K) {
    ...
    for (size_t m = 0; m < M; ++m) {
        ...
        for (size_t k = 0; k < K; ++k) {
            dst_row[k] = src_row[static_cast<size_t>(indices[k])];
        }
    }
}
...
tensor_t permute_cols(tensor_t input, tensor_t indices) {
    ...
    ASSERT(input->isContiguous() && indices->isContiguous(),
           "permute_cols requires contiguous tensors.");
    ASSERT(input->dim(1) == indices->dim(0),
           "permute_cols indices length must match input columns.");
    ...
    auto output = Tensor::create(input->shape(), input->dtype(),
                                 input->deviceType(), input->deviceId());

    switch (input->dtype()) {
    case ZEDINFER_DTYPE_F32:
        permute_cols_impl(reinterpret_cast<float *>(output->data()),
                          reinterpret_cast<const float *>(input->data()),
                          perm, M, K);
        return output;
    case ZEDINFER_DTYPE_BF16:
        permute_cols_impl(reinterpret_cast<zedinfer::bf16_t *>(output->data()),
                          reinterpret_cast<const zedinfer::bf16_t *>(input->data()),
                          perm, M, K);
        return output;
    case ZEDINFER_DTYPE_F16:
        permute_cols_impl(reinterpret_cast<zedinfer::fp16_t *>(output->data()),
                          reinterpret_cast<const zedinfer::fp16_t *>(input->data()),
                          perm, M, K);
        return output;
    default:
