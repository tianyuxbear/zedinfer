#include "backend/ops/self_attention/cpu/self_attention_cpu.hpp"
#include "utils/check.hpp"
#include "utils/types.hpp"

#include <cmath>
#include <cstddef>
#include <cstring>
#include <limits>
#include <omp.h>
#include <vector>

template <typename T>
void self_attention_(T *attn_val, const T *q, const T *k, const T *v, float scale,
                     size_t seqlen, size_t nhead, size_t dv, size_t total_len,
                     size_t nkvhead, size_t d) {

    const size_t group_size = nhead / nkvhead;
    std::memset(attn_val, 0, seqlen * nhead * dv * sizeof(T));

    if (nhead % nkvhead != 0) {
        throw std::invalid_argument("nhead must be a multiple of nkvhead");
    }

    // Pre-allocate one score buffer per thread to avoid heap allocation per (i, h)
    const int max_threads = omp_get_max_threads();
    std::vector<std::vector<float>> thread_scores(max_threads);
    for (auto &buf : thread_scores) {
        buf.resize(total_len);
    }

#pragma omp parallel for collapse(2)
    for (size_t i = 0; i < seqlen; ++i) {
        for (size_t h = 0; h < nhead; ++h) {
            const int tid = omp_get_thread_num();
            float *scores = thread_scores[tid].data();
            const size_t kvh = h / group_size;
            const size_t causal_end = (total_len - seqlen) + i + 1;

            // Step 1: compute Q*K scores
            float max_score = -std::numeric_limits<float>::infinity();
            for (size_t j = 0; j < causal_end; ++j) {
                float dot = 0.0f;
                const size_t q_base = i * nhead * d + h * d;
                const size_t k_base = j * nkvhead * d + kvh * d;

#pragma omp simd reduction(+ : dot)
                for (size_t dim = 0; dim < d; ++dim) {
                    float q_val = zedinfer::utils::cast<float>(q[q_base + dim]);
                    float k_val = zedinfer::utils::cast<float>(k[k_base + dim]);
                    dot += q_val * k_val;
                }
                scores[j] = dot * scale;
                max_score = std::max(max_score, scores[j]);
            }

            // Fill non-causal positions (not strictly necessary for correctness
            // since we only iterate up to causal_end, but avoids uninitialized reads)
            for (size_t j = causal_end; j < total_len; ++j) {
                scores[j] = -std::numeric_limits<float>::infinity();
            }

            // Step 2: softmax
            float exp_sum = 0.0f;
#pragma omp simd reduction(+ : exp_sum)
            for (size_t j = 0; j < causal_end; ++j) {
                scores[j] = std::exp(scores[j] - max_score);
                exp_sum += scores[j];
            }

            float inv_exp_sum = 1.0f / exp_sum;

            // Step 3: weighted V aggregation
            for (size_t dv_dim = 0; dv_dim < dv; ++dv_dim) {
                float out_val = 0.0f;

#pragma omp simd reduction(+ : out_val)
                for (size_t j = 0; j < causal_end; ++j) {
                    const size_t v_idx = j * nkvhead * dv + kvh * dv + dv_dim;
                    out_val += scores[j] * zedinfer::utils::cast<float>(v[v_idx]);
                }

                const size_t attn_idx = i * nhead * dv + h * dv + dv_dim;
                out_val *= inv_exp_sum;

                if constexpr (std::is_same_v<T, zedinfer::bf16_t> || std::is_same_v<T, zedinfer::fp16_t>) {
                    attn_val[attn_idx] = zedinfer::utils::cast<T>(out_val);
                } else {
                    attn_val[attn_idx] = out_val;
                }
            }
        }
    }
}

namespace zedinfer::ops::cpu {
void self_attention(std::byte *attn_val, const std::byte *q, const std::byte *k,
                    const std::byte *v, float scale, zedinferDataType_t type,
                    size_t seqlen, size_t nhead, size_t dv, size_t total_len,
                    size_t nkvhead, size_t d) {
    switch (type) {
    case ZEDINFER_DTYPE_F32:
        return self_attention_(reinterpret_cast<float *>(attn_val), reinterpret_cast<const float *>(q), reinterpret_cast<const float *>(k), reinterpret_cast<const float *>(v), scale, seqlen, nhead, dv, total_len, nkvhead, d);
    case ZEDINFER_DTYPE_BF16:
        return self_attention_(reinterpret_cast<zedinfer::bf16_t *>(attn_val), reinterpret_cast<const zedinfer::bf16_t *>(q), reinterpret_cast<const zedinfer::bf16_t *>(k), reinterpret_cast<const zedinfer::bf16_t *>(v), scale, seqlen, nhead, dv, total_len, nkvhead, d);
    case ZEDINFER_DTYPE_F16:
        return self_attention_(reinterpret_cast<zedinfer::fp16_t *>(attn_val), reinterpret_cast<const zedinfer::fp16_t *>(q), reinterpret_cast<const zedinfer::fp16_t *>(k), reinterpret_cast<const zedinfer::fp16_t *>(v), scale, seqlen, nhead, dv, total_len, nkvhead, d);
    default:
        EXCEPTION_UNSUPPORTED_DATATYPE(type);
    }
}
} // namespace zedinfer::ops::cpu
