...
template <typename T>
void mrope_3d_typed(T* xp, const int32_t* pos_t, const int32_t* pos_h, const int32_t* pos_w, int N, int H, int Dh,
                    int Dh_rot, int s0, int s1, int s2, bool interleaved, float theta) {
    ...
    for (int n = 0; n < N; ++n) {
        ...
        for (int hd = 0; hd < H; ++hd) {
            ...
            for (int pi = 0; pi < half; ++pi) {
                ...
                const float freq = 1.0f / std::pow(theta, static_cast<float>(2 * pi) / static_cast<float>(2 * half));
...
void mrope_3d(tensor_t x, tensor_t pos_ids_thw, const model::MRoPEConfig& cfg) {
    ...
    if (Dh_rot % 2 != 0) {
        --Dh_rot; // must be even for cos/sin pairing.
    }
    if (Dh_rot <= 0) {
        return;
    }
    ...
    switch (x->dtype()) {
        case ZEDINFER_DTYPE_BF16: {
            ...
            mrope_3d_typed<bf16_t>(xp, pos_t, pos_h, pos_w, N, H, Dh, Dh_rot, cfg.section[0], cfg.section[1],
                                   cfg.section[2], cfg.interleaved, cfg.theta);
            break;
        }
        case ZEDINFER_DTYPE_F32: {
            auto* xp = reinterpret_cast<float*>(x->data());
            mrope_3d_typed<float>(xp, pos_t, pos_h, pos_w, N, H, Dh, Dh_rot, cfg.section[0], cfg.section[1],
                                  cfg.section[2], cfg.interleaved, cfg.theta);
            break;
        }
        default:
