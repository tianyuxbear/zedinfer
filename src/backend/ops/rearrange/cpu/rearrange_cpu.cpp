...
template <typename T>
void rearrange_(T* out, const T* in, size_t numel, const std::vector<size_t>& out_shape,
                const std::vector<ptrdiff_t>& out_strides, const std::vector<size_t>& in_shape,
...
void rearrange(std::byte* out, const std::byte* in, zedinferDataType_t type, size_t numel,
    ...
    switch (type) {
        case ZEDINFER_DTYPE_F32:
            return rearrange_(reinterpret_cast<float*>(out), reinterpret_cast<const float*>(in), numel, out_shape,
