...
The shared logging setup applies to:
...
```text
pid=12345 INFO  2026-06-10 14:22:31.123 [tid=12345 main@151] [Server] Listening: http://127.0.0.1:8080
```
...
`chat` is interactive, so console logs are disabled by default. This prevents log lines from interleaving with:

- prompts
- user input
- streamed model output
- in-session command responses
...
```text
[HTTP] remote=127.0.0.1:52310 local=127.0.0.1:8080 method=POST target="/v1/chat/completions" version=HTTP/1.1 status=200 bytes=512 content_length=128 user_agent="curl/8.5.0"
```
...
- model/server startup status
- scheduler or serving-loop diagnostics
- environment-controlled debug dumps, such as token ids or top logits
...
Environment-controlled debug output must go through the logger. Current examples:

| Variable | Output |
|----------|--------|
| `ZEDINFER_DUMP_TOP_LOGITS` | Argmax top-logit debug lines |
| `ZEDINFER_DUMP_TOKEN_IDS` | Generated token id trace |
| `ZEDINFER_MTP_DEBUG` | MTP speculative decoding debug lines |

Keep these logs at `info` only when they are explicitly enabled by an environment variable. Otherwise prefer `debug` or `verbose` for high-volume diagnostics.
