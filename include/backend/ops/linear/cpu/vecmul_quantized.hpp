...
inline void vecmul_q8_soa(float* __restrict__ output, const float* __restrict__ input,
                          const int8_t* __restrict__ weight_q, const float* __restrict__ bias,
                          const float* __restrict__ scales, int n, int k, int group_size) {
    ...
    for (int row = 0; row < n; ++row) {
        ...
        for (int group = 0; group < num_groups; ++group) {
            ...
            for (; col + 63 < k_end; col += 64) {
                ...
                sum0
                    = _mm512_fmadd_ps(_mm512_cvtepi32_ps(_mm512_cvtepi8_epi32(w0)), _mm512_loadu_ps(input + col), sum0);
                sum1 = _mm512_fmadd_ps(_mm512_cvtepi32_ps(_mm512_cvtepi8_epi32(w1)), _mm512_loadu_ps(input + col + 16),
                                       sum1);
                sum2 = _mm512_fmadd_ps(_mm512_cvtepi32_ps(_mm512_cvtepi8_epi32(w2)), _mm512_loadu_ps(input + col + 32),
                                       sum2);
                sum3 = _mm512_fmadd_ps(_mm512_cvtepi32_ps(_mm512_cvtepi8_epi32(w3)), _mm512_loadu_ps(input + col + 48),
                                       sum3);
            }
            ...
            for (; col + 31 < k_end; col += 32) {
                ...
                sum0
                    = _mm256_fmadd_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(w0)), _mm256_loadu_ps(input + col), sum0);
                sum1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(w1)), _mm256_loadu_ps(input + col + 8),
                                       sum1);
                sum2 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(w2)), _mm256_loadu_ps(input + col + 16),
                                       sum2);
                sum3 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(w3)), _mm256_loadu_ps(input + col + 24),
                                       sum3);
            }
            ...
            for (; col < k_end; ++col) { group_dot += input[col] * static_cast<float>(weight_row[col]); }
...
inline void vecmul_q4_soa(float* __restrict__ output, const float* __restrict__ input,
                          const int32_t* __restrict__ weight_packed, const float* __restrict__ bias,
                          const float* __restrict__ scales, int n, int k, int group_size) {
    const int num_groups = k / group_size;

#pragma omp parallel for schedule(static)
    for (int row = 0; row < n; ++row) {
        const int32_t* weight_row = weight_packed + static_cast<std::size_t>(row) * (k / kInt4PackSize);
        const float* scale_row = scales + static_cast<std::size_t>(row) * num_groups;
        ...
        for (int group = 0; group < num_groups; ++group) {
            ...
            for (; offset + 63 < group_len; offset += 64) {
                const __m256i packed
                    = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(packed_group + offset / kInt4PackSize));
                ...
                const __m512i hi = _mm512_and_si512(_mm512_srli_epi16(packed_u16, 4), _mm512_set1_epi16(0x000F));
                ...
                dot_acc = _mm512_fmadd_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(w0)), in0, dot_acc);
                dot_acc = _mm512_fmadd_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(w1)), in1, dot_acc);
                dot_acc = _mm512_fmadd_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(w2)), in2, dot_acc);
                dot_acc = _mm512_fmadd_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(w3)), in3, dot_acc);
            ...
            for (; offset + 31 < group_len; offset += 32) {
                const __m128i packed
                    = _mm_loadu_si128(reinterpret_cast<const __m128i*>(packed_group + offset / kInt4PackSize));
            ...
            for (; offset < group_len; ++offset) {
                dot_product
                    += input_group[offset] * static_cast<float>(unpack_int4_unsigned(weight_row, k_start + offset));
