...
inline void pack_panel_a(const float *a_panel, float *dst, int mr, int kc, int k) {
    for (int p = 0; p < kc; ++p) {
        for (int i = 0; i < mr; ++i) {
            *dst++ = a_panel[static_cast<std::size_t>(i) * k + p];
        }
        for (int i = mr; i < kGemmMr; ++i) {
            *dst++ = 0.0f;
        }
    }
}
...
inline void pack_block_a(const float *a_block, float *dst, int mc, int kc, int k) {
    ...
    for (int i = 0; i < mc; i += kGemmMr) {
        ...
        pack_panel_a(
            a_block + static_cast<std::size_t>(i) * k,
            dst + static_cast<std::size_t>(i) * kc,
            mr, kc, k);
    }
}

template <typename LoadWeightFn>
inline void pack_panel_b(float *dst, const float *scales, int nr, int kc,
                         int group_size, int num_groups, int n_offset,
    ...
    for (int p = 0; p < kc; ++p) {
        ...
        for (int j = nr; j < kGemmNr; ++j) {
            *dst++ = 0.0f;
        }
    }
}

inline void pack_block_b_q8(const int8_t *weight, float *dst, const float *scales,
                            int nc, int kc, int k, int group_size,
                            int num_groups, int n_offset, int k_offset) {
    #pragma omp parallel for num_threads(kGemmNumThreads)
    for (int j = 0; j < nc; j += kGemmNr) {
        const int nr = std::min(kGemmNr, nc - j);
        pack_panel_b(
            dst + static_cast<std::size_t>(j) * kc, scales, nr, kc,
            group_size, num_groups, n_offset + j, k_offset,
...
inline void pack_block_b_q4(const int32_t *weight_packed, float *dst, const float *scales,
                            int nc, int kc, int k, int group_size,
                            int num_groups, int n_offset, int k_offset) {
    #pragma omp parallel for num_threads(kGemmNumThreads)
    for (int j = 0; j < nc; j += kGemmNr) {
...
inline void gemm_fma_loop(const float *a_packed, const float *b_packed,
                          __m512 accum[kGemmMr][2], int kc) {
    for (int p = 0; p < kc; ++p) {
        ...
        GEMM_FMA_ROW(0)  GEMM_FMA_ROW(1)  GEMM_FMA_ROW(2)
        GEMM_FMA_ROW(3)  GEMM_FMA_ROW(4)  GEMM_FMA_ROW(5)
        GEMM_FMA_ROW(6)  GEMM_FMA_ROW(7)  GEMM_FMA_ROW(8)
        GEMM_FMA_ROW(9)  GEMM_FMA_ROW(10) GEMM_FMA_ROW(11)
        GEMM_FMA_ROW(12) GEMM_FMA_ROW(13)
...
inline void gemm_micro_kernel(const float *a_packed, const float *b_packed,
        ...
        for (int i = 0; i < mr; ++i) {
            ...
            for (int j = 0; j < nr; ++j) {
                c[static_cast<std::size_t>(i) * n + j] +=
                    a_value * b_packed[static_cast<std::size_t>(p) * kGemmNr + j];
            }
        }
    }
}
...
inline void matmul_packed_unified(const float *a, const void *weight, float *c,
    ...
    for (int j = 0; j < n; j += kGemmNc) {
        ...
        for (int p = 0; p < k; p += kGemmKc) {
            ...
            if (num_bits == 8) {
                pack_block_b_q8(
                    reinterpret_cast<const int8_t *>(weight), g_block_b_packed,
                    scales, nc, kc, k, group_size, num_groups, j, p);
            } else {
                pack_block_b_q4(
                    reinterpret_cast<const int32_t *>(weight), g_block_b_packed,
                    scales, nc, kc, k, group_size, num_groups, j, p);
            }

            for (int i = 0; i < m; i += kGemmMc) {
                const int mc = std::min(kGemmMc, m - i);
                pack_block_a(a + static_cast<std::size_t>(i) * k + p,
                             g_block_a_packed, mc, kc, k);
                ...
                for (int jr = 0; jr < nc; jr += kGemmNr) {
                    ...
                    for (int ir = 0; ir < mc; ir += kGemmMr) {
                        ...
                        gemm_micro_kernel(
                            g_block_a_packed + static_cast<std::size_t>(kc) * ir,
                            g_block_b_packed + static_cast<std::size_t>(kc) * jr,
                            c + static_cast<std::size_t>(i + ir) * n + (j + jr),
                            mr, nr, kc, n);
                    }
                }
            }
