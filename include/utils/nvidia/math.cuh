 ...
 */
template <typename T> DEVICE_INLINE float dot_packed_128b(const T* a, const T* b);

// FP32 Specialization: 4 elements
// Already native vector size, straightforward.
template <> DEVICE_INLINE float dot_packed_128b<float>(const float* a, const float* b) {
...
template <> DEVICE_INLINE float dot_packed_128b<half>(const half* a, const half* b) {
    float4 va = load_128b(a);
    float4 vb = load_128b(b);
...
template <> DEVICE_INLINE float dot_packed_128b<cuda_bfloat16>(const cuda_bfloat16* a, const cuda_bfloat16* b) {
    float4 va = load_128b(a);
    float4 vb = load_128b(b);
