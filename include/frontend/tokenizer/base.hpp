...
struct SpecialTokens {
    ...
    bool is_special_token(const std::string& token) const { return token_to_id.count(token) > 0; }

    bool is_special_token_id(int id) const { return id_to_token.count(id) > 0; }
};
