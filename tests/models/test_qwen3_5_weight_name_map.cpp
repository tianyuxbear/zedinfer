...
TEST(Qwen3_5WeightNameMap, StripsLanguageModelPrefix) {
    EXPECT_EQ(Model::map_weight_name("model.language_model.embed_tokens.weight"), "embed_tokens.weight");
...
TEST(Qwen3_5WeightNameMap, PreservesVisionAndMtpPrefixes) {
    ...
    EXPECT_EQ(Model::map_weight_name("model.visual.blocks.0.attn.qkv.weight"), "visual.blocks.0.attn.qkv.weight");
    // MTP keys: have no "model." prefix at top, kept as-is.
    EXPECT_EQ(Model::map_weight_name("mtp.layers.0.self_attn.q_proj.weight"), "mtp.layers.0.self_attn.q_proj.weight");
    // lm_head untouched.
...
TEST(Qwen3_5WeightNameMap, Qwen3RegressionUnchanged) {
    ...
    EXPECT_EQ(Model::map_weight_name("model.layers.0.self_attn.q_proj.weight"), "layers.0.self_attn.q_proj.weight");
    EXPECT_EQ(Model::map_weight_name("model.embed_tokens.weight"), "embed_tokens.weight");
