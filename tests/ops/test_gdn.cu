...
template<typename T>
static void read_block(std::ifstream& f, std::vector<T>& vec, size_t n) {
    vec.resize(n);
...
static void run(const std::string& fixture_path) {
    ...
    auto to_dev_bf16 = [&](const std::vector<uint16_t>& host,
                           std::vector<size_t> shape) -> tensor_t {
        auto t = Tensor::create(shape, ZEDINFER_DTYPE_BF16, dev, 0);
        api->memcpy_sync(t->data(), host.data(),
                         host.size() * sizeof(uint16_t), ZEDINFER_MEMCPY_H2D);
        return t;
    };
    auto to_dev_f32 = [&](const std::vector<float>& host,
                          std::vector<size_t> shape) -> tensor_t {
        auto t = Tensor::create(shape, ZEDINFER_DTYPE_F32, dev, 0);
        api->memcpy_sync(t->data(), host.data(),
                         host.size() * sizeof(float), ZEDINFER_MEMCPY_H2D);
        return t;
    };
    ...
    auto out_t   = Tensor::create({(size_t)fx.N, (size_t)fx.Hv * fx.Dv},
                                   ZEDINFER_DTYPE_BF16, dev, 0);
    ...
    api->memcpy_sync(out_host.data(), out_t->data(),
                     out_host.size() * sizeof(uint16_t), ZEDINFER_MEMCPY_D2H);
    ...
    for (size_t i = 0; i < out_host.size(); ++i) {
        ...
        if (std::abs(got - exp) > tol) {
            if (first_bad < 0) {
                first_bad = (int)i; first_got = got; first_exp_y = exp;
            }
            ++bad_y;
        }
    }
    ...
    EXPECT_LT(bad_y, y_limit)
        << "fixture=" << fixture_path
        << " y mismatched=" << bad_y << "/" << out_host.size()
        << " (limit=" << y_limit << ")"
    ...
    api->memcpy_sync(S_host.data(), state_t->data(),
                     S_host.size() * sizeof(float), ZEDINFER_MEMCPY_D2H);
    ...
    EXPECT_LT(bad_s, s_limit)
        << "fixture=" << fixture_path
        << " state mismatched=" << bad_s << "/" << S_host.size()
        << " (limit=" << s_limit << ")"
...
TEST_F(GDNTest, DecodeN1)   { run("tests/data/gdn_fixtures/decode_n1.bin"); }
TEST_F(GDNTest, PrefillN4)  { run("tests/data/gdn_fixtures/prefill_n4.bin"); }
TEST_F(GDNTest, PrefillN12) { run("tests/data/gdn_fixtures/prefill_n12.bin"); }
