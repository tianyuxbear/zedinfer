...
static std::string get_tokenizer_path() {
    ...
    std::string base = env ? std::string(env) : "/mnt/hdd0/shared/models/deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B";
...
protected:
    ...
    void SetUp() override {
        ...
        if (!tokenizer) {
            GTEST_SKIP() << "Tokenizer not found at: " << tokenizer_json_path;
        }
    }
};
...
TEST_F(HFTokenizerTest, PromptTest) {
    ...
    std::vector<int> expected_ids = {151646, 151646, 151644, 15191, 525, 498, 30, 151645, 151648, 198};
    ...
    EXPECT_EQ(actual_input, expected_input) << "Template application result mismatch";
}
...
TEST_F(HFTokenizerTest, EncodePromptTest) {
    ...
    EXPECT_EQ(actual_ids.size(), expected_ids.size())
        << "Token count should match. actual: " << actual_ids.size() << ", expected: " << expected_ids.size();

    for (size_t i = 0; i < std::min(actual_ids.size(), expected_ids.size()); i++) {
        EXPECT_EQ(actual_ids[i], expected_ids[i])
            << "Position " << i << " token mismatch. actual: " << actual_ids[i] << ", expected: " << expected_ids[i];
    }
}

TEST_F(HFTokenizerTest, DecodePromptTest) {
    std::vector<int> input_ids = {151646, 151646, 151644, 15191, 525, 498, 30, 151645, 151648, 198};

    std::string decoded = tokenizer->decode(input_ids);

    // Decoded result should contain the user question
    EXPECT_NE(decoded.find("Who are you?"), std::string::npos) << "Decoded result should contain the original question";
}
...
TEST_F(HFTokenizerTest, BatchEncodingTest) {
    std::vector<std::string> texts = {"Hello", "How are you?", "What is AI?", "1+1=2", "Good morning!"};
...
TEST_F(HFTokenizerTest, PromptTest_ZH) {
    ...
    std::vector<int> expected_ids = {151646, 151646, 151644, 108386, 3837, 99489, 6313, 151645, 151648, 198};
...
TEST_F(HFTokenizerTest, DecodePromptTest_ZH) {
    std::vector<int> input_ids = {151646, 151646, 151644, 108386, 3837, 99489, 6313, 151645, 151648, 198};

    std::string decoded = tokenizer->decode(input_ids);

    // Decoded result should contain the user question
    EXPECT_NE(decoded.find("你好，世界！"), std::string::npos) << "Decoded result should contain the original question";
}

TEST_F(HFTokenizerTest, DecodeAnswerTest_ZH) {
    std::vector<int> input_ids
        = {151646, 151646, 151644, 108386, 3837,   99489,  6313,   151645, 151648, 198,    108386, 6313,  104198, 33464,
           39350,  10911,  16,     3837,   104139, 109944, 99663,  101214, 101037, 94432,  151649, 271,   108386, 6313,
           104198, 33464,  39350,  10911,  16,     3837,   104139, 109944, 99663,  101214, 101037, 11319, 151643};
